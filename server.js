...
import { generateSpendingPredictions, generateIncomePredictions } from './mlEngine.js'
...
app.get('/api/ml/predict-income', async (req, res) => {
  try {
    const db = readDB();
    const predictions = await generateIncomePredictions(db.students || []);
    res.json(predictions);
  } catch (error) {
    console.error('ML Income Error:', error);
    res.status(500).json({ error: 'Failed to generate ML income predictions' });
  }
});
